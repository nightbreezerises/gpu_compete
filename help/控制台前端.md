...
**密钥来源**：`control/config.yaml` 中的 `secret` 字段
...
**功能**：
- 单卡/多卡模式切换
- **多配置支持**：配置1、配置2、配置3...（无上限）
- 三级展开结构：配置 → 队列 → 进程
- 队列管理（添加、删除、折叠）
- 进程管理（添加、删除、配置）
- 命令编辑（添加、删除、修改）
- 保存/重置配置
- **运行调度器**：每个配置可独立启动/停止调度器

**组件**：
- ConfigCard：配置卡片（包含运行按钮）
- QueueCard：队列卡片
- ProcessCard：进程卡片
...
- **日志绑定功能**：命令配置页面新增"绑定日志"开关，勾选后可为每个进程绑定日志文件
- **日志查看**：状态监控页面支持查看已绑定的日志内容
- **进程级状态**：状态页面显示每个进程的详细信息（显存、命令、GPU、重试次数等）
- **移除配置文件名显示**：命令配置页面不再显示 command_gpu.txt 等文件名
- **日志独立存储**：日志绑定信息单独保存在 `control/logs/log_bindings.json`，不修改配置文件

### v2.1.0 (2025-12-03)

- **状态监控页面**：新增调度器状态监控页面
- **实时状态显示**：显示调度器 PID、模式、配置、任务进度
- **队列状态展开**：展开查看每个队列的详细状态
- **进程级停止**：支持通过 PID 停止调度器
- **自动刷新**：每 3 秒自动刷新状态

### v2.0.0 (2025-12-03)

- **多配置支持**：命令配置页支持无限数量的配置文件
- **三级展开**：配置 → 队列 → 进程 层级结构
- **调度器运行**：每个配置可独立启动/停止调度器
- **配置管理**：支持创建、删除配置
